  ...
  const loadIncome = useCallback(async () => {
    ...
    try {
      ...
      const incomeData = data.map(item => ({
        ...
        accountingMonth: item.accounting_month || item.date.substring(0, 7),
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          accounting_month: incomeData.accountingMonth || incomeData.date.substring(0, 7),
  ...
  const updateIncome = useCallback(async (id: string, updates: any) => {
    ...
    try {
      ...
      const dbUpdates: any = {
        ...
        accounting_month: updates.accountingMonth || (updates.date ? updates.date.substring(0, 7) : undefined),
