/*
  # Initial Database Setup for Digitum Finance

  1. New Tables
    - `user_profiles` - User account information and roles
    - `accounts` - Financial accounts (Bank Alfalah, Wise USD, etc.)
    - `exchange_rates` - Currency exchange rates
    - `income` - Income transactions with accounting month tracking
  ...
  3. Features
    ...
    - Auto-calculate account balances
...
-- Create income table with accounting_month field
...
BEGIN
  ...
  INSERT INTO public.user_profiles (
    id, email, name, role, permission_level, is_active, created_at, updated_at
  ) VALUES (
    NEW.id,
    NEW.email,
  ...
  INSERT INTO public.notification_settings (
    user_id, email_enabled, whatsapp_enabled, in_app_enabled, email_address,
    reminder_days, daily_digest, weekly_report
  ) VALUES (
    NEW.id, false, false, true, NEW.email, 3, false, false
  );
...
BEGIN
  ...
  UPDATE accounts
  SET balance = v_new_balance, converted_balance = v_converted_balance, updated_at = now()
  WHERE user_id = v_user_id AND name = v_account_name AND currency = v_currency;
